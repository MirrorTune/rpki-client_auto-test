...
echo "[INFO] コンテナ内で 2_1-6_check_processing_system_ratio.sh を実行します."
...
docker run --rm \
  ...
  -e RPKI_LOG_FILE="${RPKI_LOG_FILE:-/work/2_1-6_check_processing_system_ratio.log}" \
  -v "${SCRIPT_DIR}:/work" \
  -w /work \
  rpki-client-test:latest \
  2_1-6_check_processing_system_ratio.sh
