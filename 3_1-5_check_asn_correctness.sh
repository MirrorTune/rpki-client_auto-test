...
if command -v python3 >/dev/null 2>&1; then
  pass "python3 を確認しました（ASN/CSV検証に使用）"
else
  fail "python3 が見つかりません（ASN/CSV検証に必要です）"
fi
...
if [ "${PYC}" -ne 0 ]; then
  warn "既知ROA候補の照合に失敗しました（候補がCSVにない/ASN不一致/ASN範囲不正の可能性）"
  fail "既知ROA候補のいずれも条件を満たしませんでした（CSVに存在しない/ASN不一致/ASN範囲不正）"
fi
