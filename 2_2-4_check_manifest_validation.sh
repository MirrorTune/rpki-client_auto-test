...
if [ "${mft_count}" -eq 0 ]; then
  fail "キャッシュ配下に .mft が見つかりませんでした: ${RPKI_CACHE_DIR}（取得に 失敗した可能性。warmup.log を確認してください）"
fi
...
info "検証候補（最大${MAX_CANDIDATES}件から先頭${MAX_ATTEMPTS}件を試行）: list_count=${#MFT_LIST[@]}"
...
for i in $(seq 1 "${MAX_ATTEMPTS}"); do
  ...
  if grep -qiE '^Validation:[[:space:]]*N/A' "${attempt_log}"; then
    pass "マニフェストファイルの確認に成功しました: ${vline}（filemode では N/A となる場合があります）"
    success=1
    break
  fi

  if [ $rc -ne 0 ]; then
    warn "rpki-client が非0で終了しました（rc=${rc}）: ${vline}"
  else
    warn "マニフェスト検証が成功ではありません: ${vline}"
  fi
done

if [ "${success}" -eq 0 ]; then
  fail "マニフェスト検証の成功を示す 'Validation: OK' または 'Validation: N/A'  が見つかりませんでした。成果物を確認してください: ${ARTIFACT_DIR}（最後の試行ロ グ: ${last_log}）"
fi
